...
export default function Dashboard() {
    ...
    // Minor fix in filter function reference (itemTime wasn't defined in scope in original, checking fix)
    // Actually, let's keep it safe. In the original code above `itemTime` was used inside filter but defined before?
    // Checking original...
    // Ah, the original code had `const itemTime = parseMesAno(item.mesAno);` directly inside logic.
    // In my copy above I might have missed defining it if I just copy-pasted. Let me ensure it's correct.
    // Original Lines 78-81:
    // if (!item.mesAno) return true;
    // const itemTime = parseMesAno(item.mesAno);
    // if (itemTime > 0 && itemTime < cutoffTime) return false;
    // return true;
    ...
    return (
        <main className="min-h-screen flex flex-col items-center justify-start pt-24 pb-12 px-4 bg-background relative">
            ...
            <div className="absolute top-4 left-4 z-10">
                <a href="/intranet" className="flex items-center text-sm font-medium text-muted-foreground hover:text-primary transition-colors">
                    <LogOut className="w-4 h-4 mr-1 rotate-180" /> {/* Reusing LogOut for Back Arrow visual */}
